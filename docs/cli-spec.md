...
| `mark-status <id> <status>` | one record **or one trap** | status + `updated_at` (+ optional `superseded_by`) | Record lifecycle mutation (stale/disputed/superseded/…), validate-gated and reverted on failure; `--superseded-by ID` is the supersede flow. Reindexes on write. A `trap_<slug>` id resolves too: traps are blocks in the aggregate `known-traps.md` rather than one file each, so the block's `- Status:` bullet is edited in place (every other byte preserved) instead of frontmatter. Retiring a trap drops it from the resume packet and the hook pre-filter and stops it driving a `guard` verdict, while leaving it findable in `search` under its real status. A trap with no `- Status:` bullet counts as `active`. | **built** |
